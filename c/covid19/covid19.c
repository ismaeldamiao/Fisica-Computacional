...
int main(void){
   ...
   -------------------------------------------------------------------------- */
   gamma = 0.04;
   mu = 0.005; // Na Italia: mu = 0.04;
   /* --------------------------------------------------------------------------
   Valores locais, isto eh, do lugar de onde se deseja fazer a simulacao
   -------------------------------------------------------------------------- */
   sprintf(sitio, "Alagoas");
   N = 993733.0;//3337357.0; // Populacao local em numeros absolutos
   I = 6.0;// Eh preciso informar a populacao infectada no inicio da iteracao
   R0 = 3.0; // Informe o Ritmo basico de reproducao
   Tmax = 80.0; // Observar ate 150 dias
   beta = R0 * gamma;
   //beta = 0.38;
   I = I/N;
   /* ----------------------------------------------------------------------- */
   ...
   sprintf(plotSetI, "with linespoints lc rgb 'brown' pt 7 ps .3 title 'Infectados'");
   sprintf(plotSetC, "with linespoints lc rgb 'black' pt 7 ps .3 title 'Curados'");
   ...
   printf("Taxa de infecção β: %2.0lf%%\n", S * 100.0);
